...
# 加载 code/.env 中的配置（本文件位于 code/chapter6/AgentScopeDemo/）
CODE_ROOT = Path(__file__).resolve().parents[2]
load_dotenv(CODE_ROOT / ".env", override=False)

# DeepSeek 配置（OpenAI 兼容接口，均从 code/.env 读取）
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY")
DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
DEEPSEEK_MODEL = os.getenv("DEEPSEEK_MODEL", "deepseek-flash")
# 思考模式：disabled（关闭）/ enabled（开启）
# 必须关闭：DeepSeek 思考模式会返回 reasoning_content，而 agentscope 1.0.2 的
# DeepSeekMultiAgentFormatter 不支持该字段、不会回传，多轮后 DeepSeek 会报
# 400 "The reasoning_content in the thinking mode must be passed back to the API."
DEEPSEEK_THINKING = os.getenv("DEEPSEEK_THINKING", "disabled")
# 流式输出：true / false
# 必须关闭：流式返回时 AgentScope 1.0.2 解析 DeepSeek 的工具调用参数会出错
# （参数被解析成 ["response"] 这样的数组），导致每次发言都要重试一轮；
# 关闭后工具参数解析正常，不再出现 "Error in block input"。
DEEPSEEK_STREAM = os.getenv("DEEPSEEK_STREAM", "false").lower() == "true"
    ...
    async def create_player(self, role: str, character: str) -> ReActAgent:
        ...
        agent = ReActAgent(
            ...
            model=OpenAIChatModel(
                ...
                # 透传给 OpenAI SDK 的 extra_body，用于开关 DeepSeek 思考模式
                generate_kwargs={
...
async def main():
    ...
    if not DEEPSEEK_API_KEY:
        print(f"❌ 请在 {CODE_ROOT / '.env'} 中配置 DEEPSEEK_API_KEY")
        return
    
    print(f"🎮 欢迎来到三国狼人杀！（模型：{DEEPSEEK_MODEL} @ {DEEPSEEK_BASE_URL}）")
