...
配置已统一放在 `code/.env` 中，代码启动时通过 `python-dotenv` 自动加载，无需单独创建 `.env`：
...
- `DEEPSEEK_MODEL=deepseek-flash`：DeepSeek-V4.1-Flash，当前最便宜的档位。
- 说明：`deepseek-flash` 默认会返回 `reasoning_content`（思考内容），AutoGen 会忽略它、
  不会回传。本案例是纯对话、不调用工具，因此不会触发 DeepSeek 对
  `reasoning_content` 的回传校验，可以正常多轮运行。若未来给 Agent 添加工具调用
  （Function Calling），才需要处理该字段的回传问题。
