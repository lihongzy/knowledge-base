...
# 加载 code/.env 中的配置（本文件位于 code/chapter6/AutoGenDemo/）
CODE_ROOT = Path(__file__).resolve().parents[2]
load_dotenv(CODE_ROOT / ".env", override=False)

# 使用 OpenAI 兼容客户端接入 DeepSeek
...
def create_openai_model_client():
    """创建 DeepSeek 模型客户端（OpenAI 兼容接口）"""
    return OpenAIChatCompletionClient(
        model=os.getenv("DEEPSEEK_MODEL", "deepseek-flash"),
        api_key=os.getenv("DEEPSEEK_API_KEY"),
        base_url=os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com"),
        # deepseek-flash 不在 AutoGen 内置模型表里，必须手动声明模型能力，
        # 否则会抛 ValueError: model_info is required
        model_info={
            "vision": False,
